...
export default function App() {
  ...
  // Skip for co-guardians/viewers, coaches, and directors
  if (!familyAccount && !guardianMode && userRole !== 'host' && userRole !== 'coach') {
    return (
      <SetupScreen
        onComplete={async data => {
...
function HomePage({ activeTwirler, twirlerResults, twirlerComps, progress, openModal, competitions, results, setResults, invites, coachCompetitions, coaches, respondToInvite, twirlers, familyAccount, setPage, setActiveTwirlerId, pendingCoachLinks, respondToCoachLink, guardianMode, myRegistrations, setActiveCompetitionId }) {
  ...
  return (
    <div>
      ...
      {/* ── TWIRLTRACKER VIEW ── */}
      {edgeView && todayComp && (() => {
        // Use planned events as the source of truth
        const sortedPe = [...ttPlannedEvents].sort((a, b) => {
        ...
        function renderPeCard(pe, status) {
          ...
          const metaParts = [
            ...
            isDone ? null : isOnDeck ? "On Deck" : "Later",
          ].filter(Boolean).join(" · ");
          ...
          return (
            <div key={pe.id}
              ...
              <div style={{ width: isOnDeck ? 48 : 36, height: isOnDeck ? 48 : 36, borderRadius: "50%", flexShrink: 0,
                ...
                {isDone ? "✓" : (pe.event_name || "").slice(0, 2)}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: isOnDeck ? 17 : 14, fontWeight: 600, color: isDone ? "#166534" : "var(--navy)" }}>{pe.event_name}</div>
                <div style={{ fontSize: 11, color: isDone ? "#16a34a" : isOnDeck ? "var(--brand2)" : "var(--muted)", marginTop: 2 }}>
                  {isDone
                    ? (pe.category === "Movement & Compulsories"
                        ? (pe.cas_passed ? "✓ Passed" : "✗ Not Yet")
                        : `${pe.placement ? fmtPlacement(pe.placement) + " place" : "Completed"}${pe.score != null ? ` · ${pe.score}` : ""}`)
                    : metaParts}
                </div>
              </div>
              {isOnDeck && (
              ...
              {totalEvents > 0 && (
                <div style={{ marginTop: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,0.5)" }}>{completedPe.length} of {totalEvents} complete</span>
                  ...
                  <div style={{ height: 6, background: "rgba(255,255,255,0.15)", borderRadius: 999 }}>
                    <div style={{ height: "100%", borderRadius: 999, background: "var(--brand)",
                      width: `${Math.round((completedPe.length / totalEvents) * 100)}%`, transition: "width 0.4s" }} />
                  </div>
                </div>
              )}
            </div>
            ...
            {allDone ? (
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                  {completedPe.map(pe => (
                    <div key={pe.id} className="card-sm" style={{ padding: "12px 16px" }}>
                      <div style={{ fontSize: 15, fontWeight: 600, color: "var(--navy)" }}>{pe.event_name}</div>
                      <div style={{ fontSize: 13, color: "var(--slate)", marginTop: 4 }}>
                        {pe.category === "Movement & Compulsories"
                          ? (pe.cas_passed ? "✓ Passed" : "✗ Not Yet")
                          : (pe.placement ? `${fmtPlacement(pe.placement)} place` : "Completed")}
                        {pe.score != null ? ` · Score: ${pe.score}` : ""}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div className="grid-3 mb-6">
        <div className="stat-card" style={{ cursor: "pointer" }} onClick={() => setPage("competitions")}>
          <div className="stat-label">Total Competitions</div>
          <div className="stat-value">{totalComps}</div>
          <div className="stat-sub">all time</div>
        </div>
        <div className="stat-card" style={{ cursor: "pointer" }} onClick={() => { setPage("timeline"); setTimeout(() => { const el = document.getElementById("recent-wins"); if (el) el.scrollIntoView({ behavior: "smooth" }); }, 100); }}>
          ...
          {!activeTwirler ? (
            ...
            <div>
              ...
              <div>
                ...
                {peLoading ? (
                  <div style={{ textAlign: "center", padding: 24 }}><span className="baton-spinner"><BatonIcon size={28} /></span></div>
...
function UpcomingCompetitionsPage({ publicCompetitions, attendees, twirlers, activeTwirler, familyAccount, addAttendee, removeAttendee, competitionHosts, setPage, registerHost, setActiveCompetitionId, openRegistration }) {
  ...
  const upcoming = publicCompetitions
    ...
    .filter(c => c.date >= today)
    .filter(c => stateMatches(c.state, filterState))
    .filter(c => !filterOrg || c.orgId === filterOrg)
              ...
              <div key={comp.id} className="card" style={{ borderLeft: `4px solid ${orgColor(comp.orgId)}`, cursor: "pointer" }}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap mb-2">
                    ...
                    <span style={{ fontSize: 13, color: "var(--slate)" }}>📍 {[comp.city, normalizeState(comp.state) || comp.state].filter(Boolean).join(", ") || "Location TBD"}</span>
