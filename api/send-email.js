...
export default async function handler(req, res) {
  ...
  if (type === 'coach_link_request') {
    ...
    // Family account added a guardian — notify them with a direct invite link
    const { inviterName, guardianName, relationship, athleteNames, inviteUrl } = data;
    const linkUrl = inviteUrl || appUrl;
    subject = `${inviterName} invited you to their TwirlPower family account`;
    html = `
      <div style="${baseStyle}">
        ${header}
        <div style="padding: 32px;">
          ...
          <p style="font-size: 15px; color: #475569; line-height: 1.7; margin: 0 0 20px;">
            <strong>${inviterName}</strong> has invited you as a <strong>${relationship}</strong> on their TwirlPower account${athleteNames ? ` for <strong>${athleteNames}</strong>` : ''}.
          </p>
          <p style="font-size: 14px; color: #475569; line-height: 1.7; margin: 0 0 24px;">
            TwirlPower tracks baton twirling classifications, competition history, and advancement progress across USTA, NBTA, TU, and DMA — all in one place.
          </p>
          <div style="background: #f0fdfa; border: 1px solid #99f6e4; border-radius: 10px; padding: 16px 20px; margin-bottom: 24px; font-size: 14px; color: #0f172a; line-height: 1.6;">
            <strong>Click the button below</strong> to create your account and get instant access to the family's twirler data. The link is personal to you and expires in 30 days.
          </div>
          <div style="text-align: center; margin: 28px 0;">
