(function(){"use strict";const s="https://fascxnrrnsknjnojfxvv.supabase.co",r=typeof __SUPABASE_KEY__<"u"?__SUPABASE_KEY__:window.TWIRLPOWER_ANON_KEY??"";async function n(a,t={}){const e=new URLSearchParams(t).toString(),i=`${s}/rest/v1/${a}?${e}`,o=await fetch(i,{headers:{apikey:r,Authorization:`Bearer ${r}`,"Content-Type":"application/json"}});if(!o.ok)throw new Error(`Supabase error: ${o.status}`);return o.json()}const d={USTA:"#0d9488",NBTA:"#0f172a",TU:"#7c3aed",DMA:"#b45309"};function l(a){return new Date(a+"T12:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}const c=`
...
`,p=["USTA","NBTA","TU","DMA"],f=["AL","AK","AZ","AR","CA","CO","CT","DE","FL","GA","HI","ID","IL","IN","IA","KS","KY","LA","ME","MD","MA","MI","MN","MS","MO","MT","NE","NV","NH","NJ","NM","NY","NC","ND","OH","OK","OR","PA","RI","SC","SD","TN","TX","UT","VT","VA","WA","WV","WI","WY"];class g extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._org="",this._state=""}connectedCallback(){this._org=this.dataset.org??"",this._state=this.dataset.state??"",this._limit=parseInt(this.dataset.limit??"6",10),this._dir=this.dataset.directory??"https://directory.twirlpower.com",this._appUrl=this.dataset.app??"https://app.twirlpower.com",this.render(),this.fetchAndDisplay()}render(){this.shadowRoot.innerHTML=`
    ...
    `,this.shadowRoot.getElementById("org-filter").addEventListener("change",t=>{this._org=t.target.value,this.fetchAndDisplay()}),this.shadowRoot.getElementById("state-filter").addEventListener("change",t=>{this._state=t.target.value,this.fetchAndDisplay()})}async fetchAndDisplay(){const t=this.shadowRoot.getElementById("list-container");t.innerHTML='<div class="loading"><div class="spinner"></div>Loading…</div>';try{const i={select:"id,name,competition_date,city,state,org,registration_url",show_on_marketing_site:"eq.true",competition_date:`gte.${new Date().toISOString().split("T")[0]}`,order:"competition_date.asc",limit:String(this._limit)};this._org&&(i.org=`eq.${this._org}`),this._state&&(i.state=`eq.${this._state}`);const o=await n("public_competitions",i);if(!o.length){t.innerHTML='<div class="empty">No upcoming competitions found. Check back soon!</div>';return}t.innerHTML=`
        <div class="list">
          ${o.map(h=>this.renderCard(h)).join("")}
        </div>
      `}catch(e){t.innerHTML='<div class="empty">Unable to load competitions right now.</div>',console.error("TwirlPower widget error:",e)}}renderCard(t){const e=d[t.org]??"#475569",i=[t.city,t.state].filter(Boolean).join(", "),o=t.competition_date?l(t.competition_date):"Date TBD";return`
      <div class="card">
        <div class="card-stripe" style="background:${e}"></div>
        <div class="card-body">
          <div class="card-top">
            <span class="card-name">${t.name}</span>
            ${t.org?`<span class="org-badge" style="background:${e}">${t.org}</span>`:""}
          </div>
          <div class="card-meta">
            <span class="meta-item">📅 ${o}</span>
